...
const Navigator = () => {
  ...
  return (
    <div className="bg-[rgb(var(--background-rgb))] w-full min-h-full landscape:w-fit min-w-fit">
      {
        chapters &&
        chapters.map((chapter, index) => (
          <Link
            key={index}
            href={chapter.link}
            className={`flex justify-start items-center p-2 landscape:p-4 w-full h-12 landscape:h-16 ${(chapter.link === pathname) ? ' bg-black text-white dark:bg-white dark:text-black landscape:rounded-e-md font-bold' : ''}`}
